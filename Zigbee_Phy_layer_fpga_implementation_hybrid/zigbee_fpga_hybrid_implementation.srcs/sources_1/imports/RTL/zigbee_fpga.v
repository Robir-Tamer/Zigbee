...
reg                                             start_tx;
reg [$clog2(max_payload_length)-1:0]            rx_byte_count;

// Continuous Assignments
assign interleaver_valid = interleaver_valid_e || interleaver_valid_o;
assign counter_wren      = uart_vld;
assign counter_wrdata    = uart_data;

// Automatic start_tx Generation Logic
always @(posedge clk_32) begin
    if (!rst_n) begin
        ...
        start_tx <= 1'b0; 
        if (uart_vld) begin
            if (rx_byte_count == payload_length - 1) begin
                rx_byte_count <= 0;
                start_tx      <= 1'b1; 
            end else begin
