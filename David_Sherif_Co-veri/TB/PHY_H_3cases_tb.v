...
module zigbee_H_tb();
    ...
    //-- Test parameters -------------------------------------------
    parameter max_output_samples = 1000000;  //capture the output samples in large buffer
  ...
  integer test_payload_length [0:10];      // Payload length test cases
    initial begin
        ...
        // For slow mode
         test_payload_length[4] = 0;
         test_payload_length[5] = 1;
         test_payload_length[6] = 28;
         test_payload_length[7] = 127;
    end
    ...
    reg [63:0]  matlab_symbol_mapper [0:max_output_samples-1];
    reg [11:0]  matlab_demux [0:max_output_samples-1];
    ...
    initial begin
        // start log file
        log_file_TX            = $fopen("../log_H/00_TX_transcript.log"           , "w");
        log_file_QPSK          = $fopen("../log_H/06_QPSK_transcript.log"         , "w");
        log_file_interleaver   = $fopen("../log_H/04_interleaver_transcript.log"  , "w");
        log_file_symbol_mapper = $fopen("../log_H/03_symbol_mapper_transcript.log", "w");
        log_file_demux         = $fopen("../log_H/02_demux_transcript.log"        , "w");
        log_file_zero_padding  = $fopen("../log_H/01_zero_padding_transcript.log" , "w");
        log_file_dqpsk         = $fopen("../log_H/07_dqpsk_transcript.log"        , "w");
      ...
      for (test_case = 0; test_case < 8; test_case = test_case + 1) begin
        ...
        else if (test_case == 4) begin // slow mode, 0 byte
            $readmemb("../Tx_output/AI_0_Payload/Hyb_slow/00_tx_real_250Kbps.txt", matlab_tx_real);
            $readmemb("../Tx_output/AI_0_Payload/Hyb_slow/00_tx_imag_250Kbps.txt", matlab_tx_imag);
            $readmemb("../Tx_output/AI_0_Payload/Hyb_slow/06_QPSK_OUT_250K.txt", matlab_QPSK);
            $readmemb("../Tx_output/AI_0_Payload/Hyb_slow/04_interleaver_output_250K.txt", matlab_interleaver);
            $readmemb("../Tx_output/AI_0_Payload/Hyb_slow/03_symbolmapper_output_250K.txt", matlab_symbol_mapper);
            $readmemb("../Tx_output/AI_0_Payload/Hyb_slow/02_demux_output_250K.txt", matlab_demux);
            $readmemb("../Tx_output/AI_0_Payload/Hyb_slow/01_zeropadding_output_250K.txt", matlab_zero_padding);
            $readmemb("../Tx_output/AI_0_Payload/Hyb_slow/07_DQPSK_OUT_250K.txt", matlab_dqpsk);
        end
         else if (test_case == 5) begin // slow mode, 1 byte
            $readmemb("../Tx_output/AI_1_Payload/Hyb_slow/00_tx_real_250Kbps.txt", matlab_tx_real);
            $readmemb("../Tx_output/AI_1_Payload/Hyb_slow/00_tx_imag_250Kbps.txt", matlab_tx_imag);
            $readmemb("../Tx_output/AI_1_Payload/Hyb_slow/06_QPSK_OUT_250K.txt", matlab_QPSK);
            $readmemb("../Tx_output/AI_1_Payload/Hyb_slow/04_interleaver_output_250K.txt", matlab_interleaver);
            $readmemb("../Tx_output/AI_1_Payload/Hyb_slow/03_symbolmapper_output_250K.txt", matlab_symbol_mapper);
            $readmemb("../Tx_output/AI_1_Payload/Hyb_slow/02_demux_output_250K.txt", matlab_demux);
            $readmemb("../Tx_output/AI_1_Payload/Hyb_slow/01_zeropadding_output_250K.txt", matlab_zero_padding);
            $readmemb("../Tx_output/AI_1_Payload/Hyb_slow/07_DQPSK_OUT_250K.txt", matlab_dqpsk);

        end
        else if (test_case == 6) begin // slow mode, 28 byte
            $readmemb("../Tx_output/AI_28_Payload/Hyb_slow/00_tx_real_250Kbps.txt", matlab_tx_real);
            $readmemb("../Tx_output/AI_28_Payload/Hyb_slow/00_tx_imag_250Kbps.txt", matlab_tx_imag);
            $readmemb("../Tx_output/AI_28_Payload/Hyb_slow/06_QPSK_OUT_250K.txt", matlab_QPSK);
            $readmemb("../Tx_output/AI_28_Payload/Hyb_slow/04_interleaver_output_250K.txt", matlab_interleaver);
            $readmemb("../Tx_output/AI_28_Payload/Hyb_slow/03_symbolmapper_output_250K.txt", matlab_symbol_mapper);
            $readmemb("../Tx_output/AI_28_Payload/Hyb_slow/02_demux_output_250K.txt", matlab_demux);
            $readmemb("../Tx_output/AI_28_Payload/Hyb_slow/01_zeropadding_output_250K.txt", matlab_zero_padding);
            $readmemb("../Tx_output/AI_28_Payload/Hyb_slow/07_DQPSK_OUT_250K.txt", matlab_dqpsk);
        end
        else if (test_case == 7) begin // slow mode, 127 byte
            $readmemb("../Tx_output/AI_127_Payload/Hyb_slow/00_tx_real_250Kbps.txt", matlab_tx_real);
            $readmemb("../Tx_output/AI_127_Payload/Hyb_slow/00_tx_imag_250Kbps.txt", matlab_tx_imag);
            $readmemb("../Tx_output/AI_127_Payload/Hyb_slow/06_QPSK_OUT_250K.txt", matlab_QPSK);
            $readmemb("../Tx_output/AI_127_Payload/Hyb_slow/04_interleaver_output_250K.txt", matlab_interleaver);
            $readmemb("../Tx_output/AI_127_Payload/Hyb_slow/03_symbolmapper_output_250K.txt", matlab_symbol_mapper);
            $readmemb("../Tx_output/AI_127_Payload/Hyb_slow/02_demux_output_250K.txt", matlab_demux);
            $readmemb("../Tx_output/AI_127_Payload/Hyb_slow/01_zeropadding_output_250K.txt", matlab_zero_padding);
            $readmemb("../Tx_output/AI_127_Payload/Hyb_slow/07_DQPSK_OUT_250K.txt", matlab_dqpsk);
        end
        ...
        // Start transmission
        if (test_mode == 1) begin  // fast  , start_tx --> one C.C.
            start_tx = 1'b1;
            @(negedge clk);
            start_tx = 1'b0;
        end
        else if (test_mode == 0) begin  // slow  , start_tx --> two C.C.
            start_tx = 1'b1;
            repeat(2)  @(negedge clk);
            start_tx = 1'b0;
        end
        ...
        // log header for each test
        $fdisplay(log_file_zero_padding,  "\nTest_Case: %0d, Test_Mode: %0d", test_case, test_mode);
        $fdisplay(log_file_demux,         "\nTest_Case: %0d, Test_Mode: %0d", test_case, test_mode);
        $fdisplay(log_file_symbol_mapper, "\nTest_Case: %0d, Test_Mode: %0d", test_case, test_mode);
        $fdisplay(log_file_QPSK,          "\nTest_Case: %0d, Test_Mode: %0d", test_case, test_mode);
        $fdisplay(log_file_interleaver,   "\nTest_Case: %0d, Test_Mode: %0d", test_case, test_mode);
...
//demux scoreboard
            if (dut.DEMUX.valid) begin
                ...
                if (test_mode == 1)  // fast 
                    rtl_demux [demux_count] = {dut.DEMUX.e_bits[2:0] ,dut.DEMUX.o_bits[2:0]};
                else if (test_mode == 0)  // slow  
            ...
            if (dut.symbol_mapper_to_ppdu.mapper_even_valid) begin
                ...
                if (test_mode == 1)  // fast 
                    rtl_symbol_mapper [symbol_count] = {dut.symbol_mapper_to_ppdu.mapper_even_data[3:0] ,dut.symbol_mapper_to_ppdu.mapper_odd_data[3:0]};
                else if (test_mode == 0)  // slow  
...
//dqpsk scoreboard
 ...
 //tx scoreboard
    ...
    $display(" test cases no = %0d completed.",test_case);

    if (test_case == 3) begin // done testing fast mode, switch to slow
        test_mode = 0;
    end
